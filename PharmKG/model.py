...
class TransferNet(nn.Module):
    def __init__(self, args, ent2id, rel2id, triples):
        super().__init__()
        self.args = args
        self.num_steps = getattr(args, 'num_steps', 2)
        ...
        values = torch.ones(Tsize, dtype=torch.float32)
        self.Msubj = torch.sparse_coo_tensor(
            torch.stack((idx, triples[:,0])), values, torch.Size([Tsize, Esize]))
        self.Mobj = torch.sparse_coo_tensor(
            torch.stack((idx, triples[:,2])), values, torch.Size([Tsize, Esize]))
        self.Mrel = torch.sparse_coo_tensor(
            torch.stack((idx, triples[:,1])), values, torch.Size([Tsize, num_relations]))
