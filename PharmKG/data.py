...
class DataLoader(torch.utils.data.DataLoader):
    def __init__(self, input_dir, fn, bert_name, ent2id, rel2id, batch_size, training=False):
        ...
        for line in iter_file_with_progress(train_path, desc='build PharmKG adjacency', unit='B'):
            l = line.strip().split('\t')
            s = l[0].strip()
            o = l[2].strip()
        ...
        for line_no, line in enumerate(iter_file_with_progress(fn, desc='parse PharmKG questions', unit='B'), 1):
            line = line.strip()
            if line == '':
                continue
            line = line.split('\t')
            ...
            if len(line) != 2:
                continue
            question, separator, topic_marker = line[0].rpartition(' [')
            if not separator or not topic_marker.endswith(']'):
                continue
            head = topic_marker[:-1].strip()
        ...
        starts = range(0, len(data), tokenize_batch_size)
        for start in tqdm(
            starts,
            desc='tokenize PharmKG questions',
            unit='batch',
            dynamic_ncols=True,
            mininterval=1.0,
            file=sys.stdout,
...
def load_graph(input_dir):
    """加载 PharmKG 图谱，不构造问答 DataLoader。"""
    print('Read data...')
    ent2id = {}
    with open(os.path.join(input_dir, 'fbwq_full/entities.dict')) as source:
        for line in source:
            l = line.strip().split('\t')
            ent2id[l[0].strip()] = len(ent2id)
    rel2id = {}
    with open(os.path.join(input_dir, 'fbwq_full/relations.dict')) as source:
        for line in source:
            l = line.strip().split('\t')
            rel2id[l[0].strip()] = int(l[1])

    triples = []
    with open(os.path.join(input_dir, 'fbwq_full/train.txt')) as source:
        for line in source:
            l = line.strip().split('\t')
            s = ent2id[l[0].strip()]
            p = rel2id[l[1].strip()]
...
def load_data(input_dir, bert_name, batch_size):
    ...
    qa_dir = os.path.join(input_dir, 'QA_data/PharmKG')
    train_data = DataLoader(
        input_dir, os.path.join(qa_dir, 'qa_train_pharmkg.txt'), bert_name,
        ent2id, rel2id, batch_size, training=True,
    )
    dev_data = DataLoader(
        input_dir, os.path.join(qa_dir, 'qa_dev_pharmkg.txt'), bert_name,
        ent2id, rel2id, batch_size,
    )
    test_data = DataLoader(
        input_dir, os.path.join(qa_dir, 'qa_test_pharmkg.txt'), bert_name,
        ent2id, rel2id, batch_size,
    )

    return ent2id, rel2id, triples, train_data, dev_data, test_data
