...
def validate(args, model, data, triples_list, valid_edges_dict, device,
    ...
    out_path = output_path or "PharmKG/predict_result_pharmkg_path_info.jsonl"
    ...
    with torch.no_grad():
        for batch in tqdm(data, total=len(data)):
            ...
            for i in range(len(hit_score)):
                ...
                # ── MMR 路径检索指标 ──────────────────────────────────────────
                m, d = update_mmr_stats(mmr_stats, mmr_paths, gold_ids, id2rel=data.id2rel)
...
def main():
    ...
    parser.add_argument('--output_path', default=None,
                        help='Output jsonl path (default: PharmKG/predict_result_pharmkg_path_info.jsonl)')
    ...
    missing, unexpected = model.load_state_dict(torch.load(args.ckpt, weights_only=True), strict=False)
    if missing:
