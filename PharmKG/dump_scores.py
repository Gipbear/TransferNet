...
用法：
  python -m PharmKG.dump_scores \\
      --ckpt data/ckpt/PharmKG/model.pt \\
      --input_dir data/input/PharmKG \\
      --mode val \\
      --output output/score_cache/pharmkg_val.pt \\
      --topk 500
"""
import argparse
...
from .data import DataLoader, load_graph
...
def _default_log_path(output_path: str) -> str:
    ...
    stem = os.path.splitext(os.path.basename(output_abs))[0] or "pharmkg_scores"
...
def dump_scores(model, data, device, output_path, topk=500, mode="val",
    ...
    if raw_questions is None:
        raise RuntimeError(
            "DataLoader 缺少 qa_text 属性，无法保证问句与得分对齐；请更新 PharmKG/data.py。"
        )
    sample_counter = 0
...
def main():
    parser = argparse.ArgumentParser(description="PharmKG TransferNet 得分矩阵导出")
    parser.add_argument("--input_dir",  required=True,
                        help="PharmKG 数据目录（含 fbwq_full/）")
    ...
    parser.add_argument("--output",     default="output/score_cache/pharmkg_scores.pt",
                        help="缓存输出路径（.pt 文件）")
    parser.add_argument("--topk",       type=int, default=500,
                        help="每跳保存的实体得分 top-K 数量（默认 500）")
    parser.add_argument("--batch_size", type=int, default=16)
    parser.add_argument("--qa_file",    default=None,
                        help="可选 QA 文件路径；默认使用 --mode 对应的 PharmKG 分割")
    ...
    log.info("加载数据 ...")
    ent2id, rel2id, triples = load_graph(args.input_dir)
    split_files = {
        "train": "qa_train_pharmkg.txt",
        "val": "qa_dev_pharmkg.txt",
        "test": "qa_test_pharmkg.txt",
    }
    qa_file = args.qa_file or os.path.join("QA_data", "PharmKG", split_files[args.mode])
    if not os.path.isabs(qa_file):
        qa_file = os.path.join(args.input_dir, qa_file)
    log.info("使用 QA 文件: %s", qa_file)
    loader = DataLoader(
    ...
    missing, unexpected = model.load_state_dict(
        torch.load(args.ckpt, map_location="cpu", weights_only=True), strict=False
    )
    if missing:
