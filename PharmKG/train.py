...
def train(args, data_loader=load_data):
    ...
    ent2id, rel2id, triples, train_loader, dev_loader, _ = data_loader(
        args.input_dir,
        args.bert_name,
        args.batch_size,
    )
    ...
    for epoch in range(args.num_epoch):
        ...
        if (epoch + 1) % save_every == 0 or epoch + 1 == args.num_epoch:
            if args.validate_paths:
                acc = validate(args, model, dev_loader, triples_list, valid_edges_dict, device)
            else:
                acc = validate_qa_acc(model, dev_loader, device)
            logging.info(acc)
