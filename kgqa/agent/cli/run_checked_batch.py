"""Run the checked-batch KGQA agent on one question.

迁自 ``oh_my_agent/cli/run_checked_batch_agent.py``(stage3):参数面保持,
新增 ``--dataset``(实体映射默认走 dataset spec)。
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the checked-batch KGQA agent")
    parser.add_argument(
        "--dataset",
        choices=["webqsp", "metaqa"],
        default="webqsp",
        help="Dataset spec for entity mapping (default keeps WebQSP behavior)",
    )
    parser.add_argument("--question", required=True, help="Natural-language question")
    parser.add_argument("--topic_mid", required=True, help="Topic entity id (WebQSP MID or entity name)")
    ...
    parser.add_argument(
        "--entity_map",
        default="",
        help="ID->name mapping file; defaults to the dataset spec",
    )
