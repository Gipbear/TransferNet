...
def compute_answer_metrics(pred: list[str], gold: list[str]) -> dict[str, float | int | bool]:
    ...
    if not pred_set and not gold_set:
        return {
            "hit1": 1, "hit_any": 1, "precision": 1.0, "recall": 1.0,
            "f1": 1.0, "exact_match": True, "tp": 0, "pred_n": 0, "gold_n": 0,
        }
    if not pred_set or not gold_set:
        return {
            "hit1": hit1, "hit_any": hit_any, "precision": 0.0, "recall": 0.0,
            "f1": 0.0, "exact_match": False, "tp": 0,
            "pred_n": len(pred_set), "gold_n": len(gold_set),
        }
    ...
    return {
        "hit1": hit1, "hit_any": hit_any, "precision": precision, "recall": recall,
        "f1": f1, "exact_match": pred_set == gold_set, "tp": tp,
        "pred_n": len(pred_set), "gold_n": len(gold_set),
    }
...
def compute_faithfulness(
    ...
    citation_accuracy = len(cited_indices & golden_indices) / len(cited_indices) if cited_indices else 0.0
    citation_recall = len(cited_indices & golden_indices) / len(golden_indices) if golden_indices else 0.0
    effective_answers = [
...
def llm_produced_answers(
    ...
    return [
        name for name, mid in zip(pred_names, pred_mids)
