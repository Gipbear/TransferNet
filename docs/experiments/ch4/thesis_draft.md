...
| 基座模型+候选路径     |     82.23 |     90.01 |     64.07 |     33.08 |      64.25 |      69.27 |         1.83 |
...
**推理可解释性** 除了微调模型本身的性能提升，本章方法也注重模型本身的可解释性，即模型推理能否在正确回答问题的同时给出其推理路径证据。从表 4-1可以看出，与基座模型结合候选路径的结果相比，微调模型的路径引用精确率和召回率分别提升了19.83和19.39个百分点（达到84.08%和88.66%），说明模型能够更加准确地选择与问题相关的支持路径。此外，基座模型在输出答案时给出1.83%的与候选路径无关的实体作为答案，这说明模型没有忠实遵循候选路径进行推理，输出存在一定的幻觉；PFIT微调后模型输出的路径外实体率为0.00%，说明模型输出的所有答案实体均能在输入候选路径中找到对应依据，未出现超出路径上下文覆盖范围的实体输出，能够忠实依据候选路径进行回答。

本节进一步在检索质量较差的 CWQ 数据集上验证模型在候选路径中不存在正确答案时的拒答能力，结果如表 4-2 所示。

<p align="center">表4-2　CWQ拒答能力表现（%）</p>

| 方法                   |     Hit@1 |       Hit |  Macro-F1 |        EM | 路径外实体率 | 拒答精确率 | 拒答召回率 |    拒答F1 |
| :--------------------- | --------: | --------: | --------: | --------: | -----------: | ---------: | ---------: | --------: |
| 基座模型               |     48.29 | **60.10** |     42.67 |     20.87 |         1.88 |       0.00 |       0.00 |      0.00 |
| PFIT（无拒答样本）     |     55.68 |     58.91 |     53.12 |     45.74 |         0.86 |       0.00 |       0.00 |      0.00 |
| **PFIT（含拒答样本）** | **56.67** |     59.90 | **54.57** | **47.98** |     **0.03** |  **96.51** |  **68.24** | **79.95** |

**拒答能力** 在加入拒答样本前，基座模型和无拒答样本的 PFIT 微调模型对所有的问题都会强行作答，其拒答指标均为0，这说明模型一定会在候选路径中会选择至少一条路径来回答问题。相比之下，加入拒答样本并完成 PFIT 微调后，模型取得了96.51%的拒答精确率和68.24%的拒答召回率，同时在 Hit@1 等问答性能指标上也有所提升，这说明模型并非以牺牲问答性能为代价来提升拒答能力，其拒答结果具有较高的可信度。此外，在 CWQ 数据集上Hit@1等问答指标的大幅度提升，也说明 PFIT 在不同数据集上也有良好的泛化性。
