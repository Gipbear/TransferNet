...
**基线模型** 本章实验以 TransferNet 知识检索为骨干模型，并在 WebQSP 数据集上完成训练复现。为验证路径检索作为上下文知识输入的有效性，使用不作任何微调的 unsloth/meta-llama-3.1-8b-instruct-bnb-4bit 作为基座大语言模型进行问答推理。其中无论是用于检索的骨干模型还是用于推理的大语言模型在能力上都不优于对比方法所依赖的基线模型。
...
**与现有方法的总体结果对照** 表3-2展示了本章所提出的方法（RSDPR）与先进算法在WebQSP数据集上的性能对比。表中方法按是否使用知识图谱和大语言模型分为三类：KG类仅训练传统模型在知识图谱上推理并输出答案，LLM类仅依据大语言模型自身的参数化知识作答，KG+LLM类则先从知识图谱中获取证据再交由大语言模型作答。表3-2中外部方法的结果均转引自GNN-RAG，原文未报告相应口径结果的方法留空。
