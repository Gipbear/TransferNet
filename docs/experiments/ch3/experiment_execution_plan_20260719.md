...
- [x] 筛选终点融合收益案例。
- [x] 筛选 TARRS 答案覆盖或关系互补收益案例。
- [x] 筛选多样性提高但引入噪声、首条路径或下游 QA 下降的失败案例。
- [x] 保存问题、主题实体、golden、各方法路径、路径分数、关系相似度和下游答案。
- [x] 将案例与适用边界写入 `data/analysis/20260722_2040__ch3_case_analysis/README.md`，并制作 `docs/experiments/ch3/images/fig3-2_case_analysis.svg`。

**阶段结论：** 已完成。三个 WebQSP 2-hop 样本均由正式路径 JSONL 与固定 Llama 3.1 8B QA 预测按 `sample_index` 对齐后筛选：S1（605）展示终点融合将角色路径置于前列；S2（634）展示 TARRS 在重复法官路径外保留直接职位关系；F1（678）展示关系多样性从 0.4070 提升到 0.7211 后，替代演员证据使 QA `Hit@1` 从 1 降为 0。案例只用于解释全量趋势和适用边界，不替代路径/QA 的全量比较与配对 bootstrap。
...
**阶段结论：** 已完成。WebQSP 正式实验的配置—JSONL—summary—文档—代码提交追溯已归档于 `data/analysis/20260719_2259__ch3_p8_evidence_audit/`；P3 的案例、图3-2和最终结论回填已归档于 `data/analysis/20260722_2040__ch3_case_analysis/`。MetaQA 记录不进入第三章证据链。

## 当前总状态

- 已完成：P0 基线审计；P1 固定加性惩罚；P1-QA 六组 WebQSP 全量评测；P2 配对统计与同环境效率；P3 成功/失败案例与图3-2；P7 外部方法可比性核对；P8 论文证据收口。
- 当前进行：无。
- 后续：第三章实验闭环完成；P4/P5 MetaQA 与 P6/CWQ 均已暂缓，不纳入第三章。
...
- WebQSP 案例与适用边界：终点实体融合可将角色等问题所需尾实体从无关的贡献者路径中区分出来；TARRS 可在高度重复的关系模式中保留较低分但直接的职位关系，从而补足答案覆盖。但在 Darth Vader 配音案例中，TARRS 的关系多样性从 0.4070 提升到 0.7211，同时引入替代演员和电视剧证据，令固定 LLM 的 `Hit@1` 从 1 降为 0。故案例支持“覆盖与关系互补性存在收益及下游利用边界”，不支持稳定最终 QA 优势。详见 `data/analysis/20260722_2040__ch3_case_analysis/README.md` 与图3-2。
...
- 第三章最终证据边界：支持 WebQSP/TransferNet 同骨干受控路径对照、固定 Llama 3.1 8B 零样本下游收益、配对区间、同环境检索成本及案例级机制/边界解释。外部方法只作背景参照，不支持无条件 SOTA、跨数据集或跨模型通用性、路径因果忠实性或跨硬件速度优势；案例不替代全量统计，且不支持 TARRS 相对固定惩罚具有稳定最终 QA 优势。
