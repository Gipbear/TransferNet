...
- [x] 将案例与适用边界写入 `data/analysis/20260722_2040__ch3_case_analysis/README.md`，并制作 `docs/experiments/ch3/images/fig3-6_case_analysis.svg`。
...
**阶段结论：** 已完成。WebQSP 正式实验的配置—JSONL—summary—文档—代码提交追溯已归档于 `data/analysis/20260719_2259__ch3_p8_evidence_audit/`；P3 的案例、图3-6和最终结论回填已归档于 `data/analysis/20260722_2040__ch3_case_analysis/`。MetaQA 记录不进入第三章证据链。

## 当前总状态

- 已完成：P0 基线审计；P1 固定加性惩罚；P1-QA 六组 WebQSP 全量评测；P2 配对统计与同环境效率；P3 成功/失败案例与图3-6；P7 外部方法可比性核对；P8 论文证据收口。
- 当前进行：无。
...
- WebQSP 案例与适用边界：终点实体融合可将角色等问题所需尾实体从无关的贡献者路径中区分出来；TARRS 可在高度重复的关系模式中保留较低分但直接的职位关系，从而补足答案覆盖。但在 Darth Vader 配音案例中，TARRS 的关系多样性从 0.4070 提升到 0.7211，同时引入替代演员和电视剧证据，令固定 LLM 的 `Hit@1` 从 1 降为 0。故案例支持“覆盖与关系互补性存在收益及下游利用边界”，不支持稳定最终 QA 优势。详见 `data/analysis/20260722_2040__ch3_case_analysis/README.md` 与图3-6。
